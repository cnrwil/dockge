    ...
    create(socket : DockgeSocket, server : DockgeServer, agentSocket : AgentSocket) {
        ...
        agentSocket.on("deleteStack", async (name : unknown, callback) => {
            try {
                ...
                try { await stack.delete(socket); } catch (e) { server.sendStackList(); throw e; }
        ...
        agentSocket.on("requestStackList", async (callback) => {
            try { checkLogin(socket); server.sendStackList(); callbackResult({ ok: true, msg: "Updated", msgi18n: true }, callback); } catch (e) { callbackError(e, callback); }
        });
        ...
        agentSocket.on("getDockerNetworkList", async (callback) => {
            try { checkLogin(socket); const dockerNetworkList = await server.getDockerNetworkList(); callbackResult({ ok: true, dockerNetworkList }, callback); } catch (e) { callbackError(e, callback); }
        });
    }
