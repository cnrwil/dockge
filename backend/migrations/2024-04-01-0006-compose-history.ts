...
/**
 * Migration: compose_history table
 * Stores previous versions of compose.yaml so the UI can diff them.
 */
export async function up(knex: Knex): Promise<void> {
    await knex.schema.createTable("compose_history", (table) => {
        table.increments("id");
...
export async function down(knex: Knex): Promise<void> {
    await knex.schema.dropTable("compose_history");
}
