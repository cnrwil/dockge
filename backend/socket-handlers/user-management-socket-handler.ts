    ...
    create(socket: DockgeSocket, server: DockgeServer) {
        socket.on("admin:getUsers", async (callback) => {
            try {
                ...
                callback({ ok: true, users: users.map((u) => ({ id: u.id, username: u.username, role: u.role ?? "operator", active: u.active })) });
        ...
        socket.on("admin:createUser", async (data, callback) => {
            try {
                ...
                if (passwordStrength(password).value === "Too weak") throw new Error("Password is too weak.");
                ...
                user.username = username; user.password = generatePasswordHash(password); user.role = role; user.active = true;
                await R.store(user);
        ...
        socket.on("admin:updateUser", async (data, callback) => {
            try {
                ...
                if (id === socket.userID && (role !== "admin" || active === false)) throw new Error("You cannot demote or deactivate your own account.");
                const user = await R.findOne("user", " id = ? ", [id]) as User;
                if (!user) throw new Error("User not found.");
                if (role !== undefined) { if (!ROLE_LABELS[role]) throw new Error("Invalid role."); user.role = role; }
                if (active !== undefined) user.active = active;
                await R.store(user);
                if (role !== undefined || active === false) server.disconnectAllSocketClients(user.id);
        ...
        socket.on("admin:deleteUser", async (id: number, callback) => {
            try {
                checkRole(socket, "admin");
                if (id === socket.userID) throw new Error("You cannot delete your own account.");
        ...
        socket.on("admin:resetUserPassword", async (data, callback) => {
            try {
                ...
                if (passwordStrength(newPassword).value === "Too weak") throw new Error("Password is too weak.");
                const user = await R.findOne("user", " id = ? ", [id]);
                if (!user) throw new Error("User not found.");
