    ...
    create(socket: DockgeSocket, server: DockgeServer) {
        socket.on("admin:getAuditLog", async (opts: { page?: number; action?: string; username?: string }, callback) => {
            try {
                ...
                const entries = await query.orderBy("created_at", "desc").limit(PAGE_SIZE).offset(offset).select("id", "username", "action", "target", "detail", "ip", "created_at");
                callback({ ok: true, entries, total: Number(count), page, pageSize: PAGE_SIZE });
