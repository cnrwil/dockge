    ...
    create(socket : DockgeSocket, server : DockgeServer) {
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if (e instanceof Error) callback({ ok: false, msg: e.message });
            }
        });
        ...
        socket.on("loginByToken", async (token, callback) => {
            ...
            try {
                ...
                if (error.message) log.error("auth", error.message + ` IP=${clientIP}`);
        ...
        socket.on("setSettings", async (data, currentPassword, callback) => {
            try {
                ...
                if (data.globalENV && data.globalENV != "# VARIABLE=value #comment") {
                    ...
                    await fsAsync.rm(path.join(server.stacksDir, "global.env"), { recursive: true, force: true });
                }
        ...
        socket.on("disconnectOtherSocketClients", async () => {
            try {
                ...
                if (e instanceof Error) log.warn("disconnectOtherSocketClients", e.message);
            }
        });
    ...
    async login(username : string, password : string) : Promise<User | null> {
        ...
        if (user && verifyPassword(password, user.password)) {
            if (needRehashPassword(user.password)) {
                await R.exec("UPDATE `user` SET password = ? WHERE id = ? ", [ generatePasswordHash(password), user.id ]);
            }
            return user;
        }
        return null;
