...
export class DockgeServer {
    ...
    constructor() {
        ...
        let args = parse<Arguments>({
            ...
            sslCert: { type: String, optional: true },
            sslKeyPassphrase: { type: String, optional: true },
            port: { type: Number, optional: true },
            hostname: { type: String, optional: true },
            dataDir: { type: String, optional: true },
            stacksDir: { type: String, optional: true },
            enableConsole: { type: Boolean, optional: true, defaultValue: false },
        });
        ...
        try { this.indexHTML = fs.readFileSync("./frontend-dist/index.html").toString(); } catch (e) {
            if (process.env.NODE_ENV !== "development") { log.error("server", "Error: Cannot find 'frontend-dist/index.html', did you install correctly?"); process.exit(1); }
        }
        this.app = express();
        if (this.config.sslKey && this.config.sslCert) {
            log.info("server", "Server Type: HTTPS");
            this.httpServer = https.createServer({ key: fs.readFileSync(this.config.sslKey), cert: fs.readFileSync(this.config.sslCert), passphrase: this.config.sslKeyPassphrase }, this.app);
        } else {
                ...
                if (!bypass) {
                    ...
                    if (origin) {
                        try {
                            ...
                            if (host !== originURL.host) { isOriginValid = false; log.error("auth", `Origin (${origin}) does not match host (${host}), IP: ${req.socket.remoteAddress}`); }
                        } catch (e) { isOriginValid = false; log.error("auth", `Invalid origin url (${origin}), IP: ${req.socket.remoteAddress}`); }
        ...
        this.io.on("connection", async (socket: Socket) => {
            ...
            dockgeSocket.emitAgent = (event : string, ...args : unknown[]) => {
                let obj = args[0];
                if (typeof(obj) === "object") { let obj2 = obj as LooseObject; obj2.endpoint = dockgeSocket.endpoint; }
    ...
    async serve() {
        this.initDataDir();
        try { await Database.init(this); } catch (e) {
        ...
        if (!jwtSecretBean) { log.info("server", "JWT secret is not found, generate one."); jwtSecretBean = await this.initJWTSecret(); log.info("server", "Stored JWT secret into database"); }
        else { log.debug("server", "Load JWT secret from database."); }
        ...
        this.httpServer.listen(this.config.port, this.config.hostname, () => {
            if (this.config.hostname) { log.info("server", `Listening on ${this.config.hostname}:${this.config.port}`); }
            else { log.info("server", `Listening on ${this.config.port}`); }
    ...
    initDataDir() {
        ...
        if (!fs.lstatSync(this.config.dataDir).isDirectory()) throw new Error(`Fatal error: ${this.config.dataDir} is not a directory`);
        if (!fs.existsSync(this.stacksDir)) fs.mkdirSync(this.stacksDir, { recursive: true });
    ...
    disconnectAllSocketClients(userID: number | undefined, currentSocketID? : string) {
        for (const rawSocket of this.io.sockets.sockets.values()) {
            ...
            if ((!userID || socket.userID === userID) && socket.id !== currentSocketID) {
                try { socket.emit("refresh"); socket.disconnect(); } catch (e) {}
            }
        }
    }
