...
export class DockgeServer {
    ...
    constructor() {
        let unexpectedErrorHandler = (error : unknown) => { console.trace(error); console.error("If you keep encountering errors, please report to https://github.com/louislam/dockge"); };
        ...
        try { this.indexHTML = fs.readFileSync("./frontend-dist/index.html").toString(); } catch (e) { if (process.env.NODE_ENV !== "development") { log.error("server", "Error: Cannot find 'frontend-dist/index.html', did you install correctly?"); process.exit(1); } }
        this.app = express();
        if (this.config.sslKey && this.config.sslCert) { log.info("server", "Server Type: HTTPS"); this.httpServer = https.createServer({ key: fs.readFileSync(this.config.sslKey), cert: fs.readFileSync(this.config.sslCert), passphrase: this.config.sslKeyPassphrase }, this.app); }
        else { log.info("server", "Server Type: HTTP"); this.httpServer = http.createServer(this.app); }
        ...
        this.app.get("*", async (_req, res) => { res.send(this.indexHTML); });
        let cors = isDev ? { origin: "*" } : undefined;
        this.io = new socketIO.Server(this.httpServer, { cors, allowRequest: (req, callback) => {
            let isOriginValid = true;
            const bypass = isDev || process.env.UPTIME_KUMA_WS_ORIGIN_CHECK === "bypass";
            if (!bypass) {
                let host = req.headers.host; let origin = req.headers.origin;
                if (origin) { try { let originURL = new URL(origin); if (host !== originURL.host) { isOriginValid = false; log.error("auth", `Origin (${origin}) does not match host (${host}), IP: ${req.socket.remoteAddress}`); } } catch (e) { isOriginValid = false; log.error("auth", `Invalid origin url (${origin}), IP: ${req.socket.remoteAddress}`); } }
                else { log.info("auth", `Origin is not set, IP: ${req.socket.remoteAddress}`); }
            } else { log.debug("auth", "Origin check is bypassed"); }
            callback(null, isOriginValid);
        }});
        this.io.on("connection", async (socket: Socket) => {
            let dockgeSocket = socket as DockgeSocket;
            dockgeSocket.instanceManager = new AgentManager(dockgeSocket);
            dockgeSocket.emitAgent = (event : string, ...args : unknown[]) => { let obj = args[0]; if (typeof(obj) === "object") { let obj2 = obj as LooseObject; obj2.endpoint = dockgeSocket.endpoint; } dockgeSocket.emit("agent", event, ...args); };
            dockgeSocket.endpoint = typeof socket.request.headers.endpoint === "string" ? socket.request.headers.endpoint : "";
            if (dockgeSocket.endpoint) { log.info("server", "Socket connected (agent), as endpoint " + dockgeSocket.endpoint); } else { log.info("server", "Socket connected (direct)"); }
            this.sendInfo(dockgeSocket, true);
            if (this.needSetup) { log.info("server", "Redirect to setup page"); dockgeSocket.emit("setup"); }
            ...
            if (await Settings.get("disableAuth")) { log.info("auth", "Disabled Auth: auto login to admin"); this.afterLogin(dockgeSocket, await R.findOne("user") as User); dockgeSocket.emit("autoLogin"); } else { log.debug("auth", "need auth"); }
            dockgeSocket.on("disconnect", () => { log.info("server", "Socket disconnected!"); dockgeSocket.instanceManager.disconnectAll(); });
        });
    ...
    async afterLogin(socket : DockgeSocket, user : User) {
        ...
        try { this.sendStackList(); } catch (e) { log.error("server", e); }
    ...
    async serve() {
        this.initDataDir();
        try { await Database.init(this); } catch (e) { if (e instanceof Error) log.error("server", "Failed to prepare your database: " + e.message); process.exit(1); }
        let jwtSecretBean = await R.findOne("setting", " `key` = ? ", ["jwtSecret"]);
        if (!jwtSecretBean) { log.info("server", "JWT secret is not found, generate one."); jwtSecretBean = await this.initJWTSecret(); } else { log.debug("server", "Load JWT secret from database."); }
        ...
        if (userCount == 0) { log.info("server", "No user, need setup"); this.needSetup = true; }
        this.httpServer.listen(this.config.port, this.config.hostname, () => {
            if (this.config.hostname) { log.info("server", `Listening on ${this.config.hostname}:${this.config.port}`); } else { log.info("server", `Listening on ${this.config.port}`); }
            Cron("*/10 * * * * *", { protect: true }, () => { this.sendStackList(true); });
    ...
    async sendInfo(socket : Socket, hideVersion = false) {
        ...
        if (!hideVersion) { versionProperty = packageJSON.version; latestVersionProperty = checkVersion.latestVersion; isContainer = (process.env.DOCKGE_IS_CONTAINER === "1"); }
        socket.emit("info", { version: versionProperty, latestVersion: latestVersionProperty, isContainer, primaryHostname: await Settings.get("primaryHostname") });
    }
    async getClientIP(socket : Socket) : Promise<string> {
        let clientIP = socket.client.conn.remoteAddress ?? "";
        if (await Settings.get("trustProxy")) { const ff = socket.client.conn.request.headers["x-forwarded-for"]; if (typeof ff === "string") return ff.split(",")[0].trim(); if (typeof socket.client.conn.request.headers["x-real-ip"] === "string") return socket.client.conn.request.headers["x-real-ip"]; }
    ...
    checkTimezone(timezone : string) { try { dayjs.utc("2013-11-18 11:55").tz(timezone).format(); } catch (e) { throw new Error("Invalid timezone:" + timezone); } }
    ...
    async sendStackList(useCache = false) {
        let socketList = this.io.sockets.sockets.values(); let stackList;
        for (let socket of socketList) { let d = socket as DockgeSocket; if (d.userID) { if (!stackList) stackList = await Stack.getStackList(this, useCache); let map : Map<string, object> = new Map(); for (let [n, s] of stackList) map.set(n, s.toSimpleJSON(d.endpoint)); log.debug("server", "Send stack list to user: " + d.id + " (" + d.endpoint + ")"); d.emitAgent("stackList", { ok: true, stackList: Object.fromEntries(map) }); } }
    }
    ...
    async shutdownFunction(signal : string | undefined) { log.info("server", "Shutdown requested"); log.info("server", "Called signal: " + signal); await Database.close(); Settings.stopCacheCleaner(); }
    finalFunction() { log.info("server", "Graceful shutdown successful!"); }
    disconnectAllSocketClients(userID: number | undefined, currentSocketID? : string) { for (const rawSocket of this.io.sockets.sockets.values()) { let socket = rawSocket as DockgeSocket; if ((!userID || socket.userID === userID) && socket.id !== currentSocketID) { try { socket.emit("refresh"); socket.disconnect(); } catch (e) {} } } }
    isSSL() { return this.config.sslKey && this.config.sslCert; }
    getLocalWebSocketURL() { const protocol = this.isSSL() ? "wss" : "ws"; const host = this.config.hostname || "localhost"; return `${protocol}://${host}:${this.config.port}`; }
}
