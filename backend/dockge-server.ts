...
export class DockgeServer {
    ...
    constructor() {
        let ueh = (error : unknown) => { console.trace(error); console.error("If you keep encountering errors, please report to https://github.com/louislam/dockge"); };
        process.addListener("unhandledRejection", ueh); process.addListener("uncaughtException", ueh);
        ...
        this.config.sslKey = args.sslKey || process.env.DOCKGE_SSL_KEY || undefined; this.config.sslCert = args.sslCert || process.env.DOCKGE_SSL_CERT || undefined; this.config.sslKeyPassphrase = args.sslKeyPassphrase || process.env.DOCKGE_SSL_KEY_PASSPHRASE || undefined;
        this.config.port = args.port || Number(process.env.DOCKGE_PORT) || 5001; this.config.hostname = args.hostname || process.env.DOCKGE_HOSTNAME || undefined; this.config.dataDir = args.dataDir || process.env.DOCKGE_DATA_DIR || "./data/"; this.config.stacksDir = args.stacksDir || process.env.DOCKGE_STACKS_DIR || defaultStacksDir; this.config.enableConsole = args.enableConsole || process.env.DOCKGE_ENABLE_CONSOLE === "true" || false; this.stacksDir = this.config.stacksDir;
        log.debug("server", this.config); this.packageJSON = packageJSON as PackageJson;
        ...
        this.app = express();
        if (this.config.sslKey && this.config.sslCert) { this.httpServer = https.createServer({ key: fs.readFileSync(this.config.sslKey), cert: fs.readFileSync(this.config.sslCert), passphrase: this.config.sslKeyPassphrase }, this.app); }
        ...
        this.io = new socketIO.Server(this.httpServer, { cors, allowRequest: (req, cb) => { let v = true; if (!(isDev || process.env.UPTIME_KUMA_WS_ORIGIN_CHECK === "bypass")) { let o = req.headers.origin; if (o) { try { if (req.headers.host !== new URL(o).host) { v = false; } } catch { v = false; } } } cb(null, v); } });
        this.io.on("connection", async (socket: Socket) => {
            let d = socket as DockgeSocket; d.instanceManager = new AgentManager(d);
            d.emitAgent = (event : string, ...args : unknown[]) => { let obj = args[0]; if (typeof(obj) === "object") { (obj as LooseObject).endpoint = d.endpoint; } d.emit("agent", event, ...args); };
            d.endpoint = typeof socket.request.headers.endpoint === "string" ? socket.request.headers.endpoint : "";
            this.sendInfo(d, true);
            if (this.needSetup) d.emit("setup");
            for (const sh of this.socketHandlerList) sh.create(d, this);
            let as = new AgentSocket(); for (const sh of this.agentSocketHandlerList) sh.create(d, this, as);
            this.agentProxySocketHandler.create2(d, this, as);
            if (await Settings.get("disableAuth")) { this.afterLogin(d, await R.findOne("user") as User); d.emit("autoLogin"); }
            d.on("disconnect", () => { log.info("server", "Socket disconnected!"); d.instanceManager.disconnectAll(); });
        });
    ...
    async getClientIP(socket : Socket) : Promise<string> { let ip = socket.client.conn.remoteAddress ?? ""; if (await Settings.get("trustProxy")) { const ff = socket.client.conn.request.headers["x-forwarded-for"]; if (typeof ff === "string") return ff.split(",")[0].trim(); if (typeof socket.client.conn.request.headers["x-real-ip"] === "string") return socket.client.conn.request.headers["x-real-ip"]; } return ip.replace(/^::ffff:/, ""); }
    async getTimezone() { try { if (process.env.TZ) { this.checkTimezone(process.env.TZ); return process.env.TZ; } } catch {} const tz = await Settings.get("serverTimezone"); try { if (tz) { this.checkTimezone(tz); return tz; } } catch {} try { const g = dayjs.tz.guess(); if (g) { this.checkTimezone(g); return g; } return "UTC"; } catch { return "UTC"; } }
    ...
    getLocalWebSocketURL() { const p = this.isSSL() ? "wss" : "ws"; const h = this.config.hostname || "localhost"; return `${p}://${h}:${this.config.port}`; }
}
