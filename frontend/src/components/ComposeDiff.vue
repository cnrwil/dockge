<template>
  <div class="shadow-box big-padding">
    <div class="d-flex justify-content-between align-items-center mb-2">
      <h6 class="mb-0">Changes since last save</h6>
      <button class="btn btn-sm btn-normal" @click="$emit('close')">Close</button>
    </div>
    <div v-if="!lines.length" class="text-muted" style="font-size:0.85em">No changes.</div>
    <div v-else class="diff-view">
      <div v-for="(line, i) in lines" :key="i" :class="lineClass(line)" class="diff-line">
...
.diff-add { background: rgba(116, 194, 255, 0.15); color: $primary; }
.diff-remove { background: rgba(220, 53, 69, 0.15); color: $danger; }
.diff-context { color: $dark-font-color; }
</style>
