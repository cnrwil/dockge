<template>
  <div>
    ...
    <div class="d-flex flex-wrap gap-1 mb-2">
      <span v-for="dep in dependencies" :key="dep" class="badge bg-secondary d-flex align-items-center gap-1">
        {{ dep }}
        <button type="button" class="btn-close btn-close-white" style="font-size:0.55em" @click="removeDep(dep)" />
      </span>
      <span v-if="dependencies.length === 0" class="text-muted" style="font-size:0.85em">None</span>
    </div>
    <div class="input-group input-group-sm" style="max-width:300px">
      ...
      <button class="btn btn-outline-primary" :disabled="!selected" @click="addDep">Add</button>
    </div>
...
export default defineComponent({
  ...
  methods: {
    load() {
      this.$root?.getSocket().emit("getStackDependencies", this.stackName, (res: any) => {
    ...
    addDep() {
      ...
      this.$root?.getSocket().emit("setStackDependencies", this.stackName, updated, (res: any) => {
