<template>
  <div>
    <h6 class="mb-2">Depends On</h6>
    <div class="d-flex flex-wrap gap-1 mb-2">
      <span v-for="dep in dependencies" :key="dep" class="badge bg-secondary d-inline-flex align-items-center gap-1">
        {{ dep }}
        <button type="button" class="btn-close btn-close-white" style="font-size:0.5em" @click="removeDep(dep)" />
      </span>
      <span v-if="!dependencies.length" class="text-muted" style="font-size:0.85em">None</span>
    </div>
    <div class="input-group input-group-sm" style="max-width:320px">
      ...
      <button class="btn btn-normal" :disabled="!selected" @click="addDep">Add</button>
    </div>
...
export default defineComponent({
  ...
  mounted() { this.load(); },
  methods: {
    load() { (this.$root as any)?.getSocket().emit("getStackDependencies", this.stackName, (res: any) => { if (res.ok) this.dependencies = res.dependencies; }); },
    addDep() {
      ...
      (this.$root as any)?.getSocket().emit("setStackDependencies", this.stackName, updated, (res: any) => {
        if (res.ok) { this.dependencies = updated; this.selected = ""; } else this.error = res.msg;
      });
    },
    removeDep(dep: string) {
      const updated = this.dependencies.filter((d) => d !== dep);
      (this.$root as any)?.getSocket().emit("setStackDependencies", this.stackName, updated, (res: any) => {
        if (res.ok) this.dependencies = updated; else this.error = res.msg;
      });
    },
  },
});
